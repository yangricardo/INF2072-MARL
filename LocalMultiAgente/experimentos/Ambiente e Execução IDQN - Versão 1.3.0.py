"""
IMPLEMENTAÇÃO CORRIGIDA PARA GOOGLE COLAB - VERSÃO CORRIGIDA
- Corrigido erro de tostring_rgb
- Adicionada função de avaliação e vídeo
...
class WarehouseEnv(gym.Env):
    ...
    def render_frame(self):
        """Renderiza um único frame como array RGB - VERSÃO CORRIGIDA"""
        ...
        # Método correto para obter array RGB em versões recentes do matplotlib
        try:
...
# ==================== FUNÇÃO DE AVALIAÇÃO E VÍDEO ====================
def evaluate_and_record_video(agents, config, save_dir, num_episodes=1):
    """Avalia os agentes e grava um vídeo - VERSÃO CORRIGIDA"""
    ...
    print(f"\n{'='*60}")
    print(f"🎬 AVALIAÇÃO E GERAÇÃO DE VÍDEO")
    print(f"{'='*60}")
    ...
    for episode in range(num_episodes):
        ...
        for agent in agents:
            original_epsilon = agent.epsilon if hasattr(agent, 'epsilon') else agent.get_epsilon()
            if hasattr(agent, 'epsilon'):
                agent.epsilon = 0.05
            else:
                # Para agentes que usam steps_done
                agent.steps_done = agent.config.EPSILON_DECAY_STEPS
        
        while True:
            # Capturar frame - usando método corrigido
            try:
                frame = env.render_frame()
                frames.append(frame)
            except Exception as e:
                print(f"  ⚠️ Erro ao capturar frame: {e}")
                # Continuar mesmo sem frame
            
            # Selecionar ações (sem exploração)
            actions = []
            for agent in agents:
                with torch.no_grad():
        ...
        print(f"  ✅ Episódio {episode + 1} | Steps: {step} | Reward: {episode_reward:.2f} | Entregas: {episode_deliveries}/4")
    ...
    if frames:
        ...
        try:
            ...
            try:
                ...
                print(f"✅ Vídeo salvo (alternativo) em: {video_path}")
            except ImportError:
                print("⚠️ moviepy não disponível. Vídeo não foi salvo.")
...
# ==================== FUNÇÃO PRINCIPAL ====================
def run_multi_session_training(num_sessions=2):
    ...
    # Gerar vídeo após o treinamento
    results_dir = base_dir / "final_results"
    os.makedirs(results_dir, exist_ok=True)
    
    print(f"\n{'='*60}")
    print(f"🎬 GERANDO VÍDEO DOS MOVIMENTOS DOS ROBÔS")
    print(f"{'='*60}")
    
    video_path, episode_stats = evaluate_and_record_video(agents, config, results_dir, num_episodes=1)
...
if __name__ == "__main__":
    ...
    OptimizedConfig.SAVE_CHECKPOINT_EVERY = 500
    
    # Número de sessões (cada sessão = 1500 episódios)
    NUM_SESSIONS = 1  # Reduzido para teste rápido
    
    try:
        agents, consolidated_metrics, video_path = run_multi_session_training(num_sessions=NUM_SESSIONS)
        ...
        if video_path:
            ...
            print("\n⚠️ Vídeo não foi gerado devido a erros.")
